...
public:
		...
		, m_firstLine(true), m_blankLineTransition(0)
	{
		...
		if (m_algorithm == "DSA" || m_algorithm == "ECDSA")
		{
			...
			m_trigger = "V";
		else if (m_algorithm == "CRYPTOPP_RSA")
...
protected:
	...
	bool IsolatedMessageEnd(bool blocking)
	{
		...
		{
			if (!m_line.empty() && ((m_algorithm == "CRYPTOPP_RSA" && m_test != "Gen") || m_algorithm == "RNG" || m_algorithm == "HMAC" || m_algorithm == "SHA" || (m_algorithm == "ECDSA" && m_test != "KeyPair") || (m_algorithm == "DSA" && (m_test == "PQGVer" || m_test == "SigVer"))))
			{
...
int FIPS_140_AlgorithmTest(int argc, char **argv)
{
	...
	if (algorithm == "auto")
	{
		string algTable[] = {"AES", "ECDSA", "DSA", "HMAC", "RNG", "CRYPTOPP_RSA", "TDES", "SKIPJACK", "SHA"};	// order is important here
	...
	try
	{
		...
		else if (algorithm == "CRYPTOPP_RSA")
