...
应用程序向导已为您创建了此 NetComm 项目，作为编写动态链接库 (DLL) 
的起点。
...
NetComm.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(NetComm.h)
        GUID 定义			(NetComm_i.c)
        类型库				(NetComm.tlb)
        封送处理代码			(NetComm_p.c 和 
                                           dlldata.c）

NetComm.h
    此文件包含 NetComm.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 NetComm.pch 的预编译头 (PCH) 文件和
