...
void CTCPServer::StartListenClient()
{
	sockaddr_in from;
	...
	Param->TP_pTCPServer = this;
	Param->TP_socket      = m_SClient;
	hThread=::CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE)CTCPServer::ServerRecThread, Param, 0, &dwThreadId);

}

int CTCPServer::SendMultiplyMessagePort(string sMessage)
{
	int iStat = 0;
...
int CTCPServer::SendSingleMessagePort(PSendMsg pMessage)
{
	...
	DWORD dwCrc32 = 0;
	OutputAReceiveMsg(pMessage->TP_string);
	...
	case 1:
		{
			printf("发送了命令777777777777777777777777777777777777\n");
			if(TRUE){
				CComBSTR bstrVal;
				...
				if(NULL != spiJsonService)
				{
					...
					spiJsonService->put_IntValue(CComBSTR(JSON_STRING_CMD_ID),7);
...
int CTCPServer::RecClient(SOCKET sRecSocket)
{
	char JsonBuff[4096];
...
UINT CTCPServer::ServerRecThread(LPVOID pParam)
{
	PThreadParam Param = (PThreadParam)pParam;

	CTCPServer* pTCPServer = Param->TP_pTCPServer;
	SOCKET sRecSocket        = Param->TP_socket;
	while(1)
	{
		if(pTCPServer->RecClient(sRecSocket))
			break;
	}
	return 0;
}

UINT CTCPServer::ServerListenThread(LPVOID pParam)
{
	CTCPServer* pTCPServer = (CTCPServer*)pParam;

	while(1)
		pTCPServer->StartListenClient();


	return 0;
}

BOOL CTCPServer::GetInterfacePtr(CComPtr <IJsonService>& spiJsonService)
{
	if(NULL == spiJsonService)
		spiJsonService = CDbHelper::GetJsonService();
	if(NULL != spiJsonService)
...
void CTCPServer::Get_CmdContext(const char* bstrContent,BSTR bstrKeyName,CString& strJson)
{
	CComPtr <IJsonService> spiJsonService = NULL;
...
void CTCPServer::Get_CmdContext(const char* bstrContent,BSTR bstrKeyName,LONGLONG& intJson)
{
	CComPtr <IJsonService> spiJsonService = NULL;
	BOOL bRet = GetInterfacePtr(spiJsonService);
	if (bRet)
...
void CTCPServer::OutputAReceiveMsg(const char *JsonBuff)
{
	CComPtr <IJsonService> spiJsonService = NULL;
