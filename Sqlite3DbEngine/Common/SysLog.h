...
public:
	...
	BOOL CreateUDPCommunication();	//创建与SysLog的通信


	BOOL GetLogDataFromDB(BOOL SetSyslogFlag = FALSE);		//从数据库中获取日志信息

	BOOL HandleSysLogFile();		//处理SysLog文件

private:

	BOOL ConvertPRIToSysLog(enumFACILITY nFacility, enumSEVERITY nSeverity, CString& strPRI);		//转换SysLog 的PRI段

	BOOL ParseAndConvertLogDateToSysLog(DOUBLE fTime, CString& strDate, BOOL bYear = TRUE);	//解析日志并转换为syslog格式

	BOOL ParseSysLogConfigFile(BYTE* pchSysLog);		//解析配置文件，获取SysLog服务器IP

	BOOL GetMonthString(int nMonth,CString& strMonth);	//获取月份字符串

private:
	char m_ServerMajorIP[64]; // 主服务IP
	char m_ServerMinorIP[64]; // 次服务IP
	char m_ServerSpareIP[64]; // 备用服务IP
	int m_ServerPort;
