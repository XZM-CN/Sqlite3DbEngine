========================================================================
    活动模板库 : HostService 项目概述
========================================================================

应用程序向导已为您创建了此 HostService 项目，作为编写服务 (EXE) 的起点。
...
HostService.idl
    ...
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明		(HostService.h)
        GUID 定义			(HostService_i.c)
        类型库				(HostService.tlb)
        封送处理代码			(HostService_p.c 和 
                                           dlldata.c）

HostService.h
    此文件包含 HostService.idl 中定义的项目的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 HostService.pch 的预编译头 (PCH) 文件和
...
HostServiceps.vcxproj
    此文件是用于生成代理/存根 (stub) DLL 的项目文件（若有必要）。主项目中的 IDL 
    文件必须至少包含一个接口，并且在生成代理/存根 (stub) DLL 之前必须先编译 IDL 
    文件。此进程生成 dlldata.c、HostService_i.c 和 
    HostService_p.c，这些文件是生成代理/存根 (stub) DLL 所必需的。

HostServiceps.vcxproj.filters
