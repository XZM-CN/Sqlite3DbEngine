...
BOOL CSysLog::CreateUDPCommunication()
{
	...
	//创建阻塞模式下的UDP客户端
	m_pNetSock = new CNetSocket(FALSE, enumDgram, enumBlock);
	if (m_pNetSock == NULL)
	{
		return FALSE;
	}
	//UDP不需要IP和端口
	m_sock = m_pNetSock->Start("192.168.0.62",514);
...
//从数据库中获取日志信息
BOOL CSysLog::GetLogDataFromDB(BOOL SetSyslogFlag /*= FALSE*/)
{
	BOOL bRet = TRUE;
	HRESULT hRet = 0;
	CString strSendData;
	...
	if(NULL != spiSqlite3Statement)
	{
		...
		while(SUCCEEDED(spiSqlite3Statement->NextRow()))
		{
			...
			CString strTemp;
			//转换格式
			if(ConvertPRIToSysLog(enumUserLevelMessages, enumInformational, strTemp))
			{
				strSendData += strTemp;
				strTemp.Empty();
			}
			...
			if(ParseAndConvertLogDateToSysLog(dLTime, strTemp))
			{
				strSendData += strTemp;
				strTemp.Empty();
			}
			...
			//SetSyslogFlag决定是否更新当前上传标志
			if(SetSyslogFlag)
			{
				CString strSqlBuff;
				strSqlBuff.Format(_T("UPDATE [%s] SET SysLogFlag=%d WHERE LID=%ld"),\
			...
			if (m_pNetSock != NULL)
			{
				...
				if (nSendLen > 0 && pSendData != NULL)
				{
					...
					m_pNetSock->SendDataTo(m_sock, pSendData, nSendLen,m_ServerSpareIP,UDPSYSLOGPORT);
	...
	if(NULL != spiSqlite3Statement)
	{
		...
		{
			DOUBLE dLTime = 0.0;
			long nType = 0,nFlag = 0,nCount = 0,nLogID = 0,nSyncFlag = 0,nSysLogFlag = 0;
			CString strLogType;

			spiSqlite3Statement->GetValueInt(8,&nSysLogFlag);
			if (nSysLogFlag)
			{
				//此条已经发送
				continue;
			...
			CString strTemp;
			//转换格式
			if(ConvertPRIToSysLog(enumKernelMessages, enumWarning, strTemp))
			{
				strSendData += strTemp;
				strTemp.Empty();
			}
	...
	if(NULL != spiSqlite3Statement)
	{
		...
		while(SUCCEEDED(spiSqlite3Statement->NextRow()))
		{
			...
			CString strTemp;
			//转换格式
			if(ConvertPRIToSysLog(enumSystemDaemons, enumNotice, strTemp))
			{
				strSendData += strTemp;
				strTemp.Empty();
			}
			...
			if(ParseAndConvertLogDateToSysLog(fTime, strTemp))
			{
				strSendData += strTemp;
				strTemp.Empty();
			}
...
//转换SysLog 的PRI段
BOOL CSysLog::ConvertPRIToSysLog(enumFACILITY nFacility, enumSEVERITY nSeverity, CString& strPRI)
{
	BOOL bRet = TRUE;
	int nFRI = 0;
...
//解析日志并转换为syslog格式
BOOL CSysLog::ParseAndConvertLogDateToSysLog(DOUBLE fTime, CString& strDate, BOOL bYear)
{
	BOOL bRet = TRUE;
	CString strTemp;
	//解析时间，分离年月日时分秒
...
//解析配置文件，获取SysLog服务器IP
BOOL CSysLog::ParseSysLogConfigFile(BYTE* pchSysLog)
{
	BOOL bRet = FALSE;
		...
		if (!strStart.IsEmpty())
		{
			...
			if (nLen != -1)
			{
				...
				while (nLen != -1)
				{
					...
					else if (0 == strIPName.CompareNoCase(SERVERSPAREIP))
					{
						char* ptempIP = NULL;
						int nSendLen = CBaseFuncLib::Us2ToChar(strIP, &ptempIP);
						if (nSendLen > 0 && nSendLen < 64 && ptempIP != NULL)
						{
							...
							memcpy(m_ServerSpareIP, ptempIP, nSendLen);
							delete[] ptempIP;
						}
