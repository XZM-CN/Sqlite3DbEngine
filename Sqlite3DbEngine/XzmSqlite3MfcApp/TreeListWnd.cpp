...
// ####################################################################################################
//
// File:	TreeListWnd.cpp
// Author:	XieZM
// Created: 2014/10/24 
//
// Description:  a Tree-List-Wnd for MFC classes implementation file. 
// ---------------------------------------------------------------------------------------------------
...
typedef struct xzmTreeListData
{
...
static void CreateStateImageList(TreeListData *pData,int iMode)
{
	...
	if(pOpenThemeData)									// Über Thema zeichnen
	{
		if(!pData->hThemeBt)
		{
			pData->hThemeBt = pOpenThemeData(pData->hWnd,TEXT("BUTTON"));
		}
...
static int TreeListEndLabelEdit(TreeListData *pData,int iMode)
{
	...
	{													// Neuen Text eingeben
		...
		else
		{
			...
			//edit by XieZM for set item data then send message
			UNLOCK(pData);		
			ShowWindow(pData->hEdit,SW_HIDE);
			lRet=SendNotify(pData,&sNotify.hdr);
			LOCK(pData);
...
static HWND TreeListEditLabel(TreeListData *pData,unsigned uItem,unsigned uSub)
{
	...
	{
		pSetWindowTheme(pData->hEdit,TEXT(""), TEXT(""));
	}
	...
	SetWindowPos (pData->hEdit,HWND_TOP,sRect.left,sRect.top,iPixels,uHeight,SWP_SHOWWINDOW);
...
static LRESULT CALLBACK TreeListProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_CREATE:												// Das Fenster erzeugen
		...
		if(pOpenThemeData)									// Soll ein Thema angezeigt werden
			pData->hTheme = pOpenThemeData(hWnd,TEXT("TREEVIEW"));
